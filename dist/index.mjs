...
function convertToDecimal(num, decimalPlaces = 5) {
  Decimal2.set({ precision: decimalPlaces, rounding: Decimal2.ROUND_UP });
...
async function requestRegistrarDetails({
  ...
  try {
    ...
    return {
      ...
      fee_percentage: new Decimal3(parseField(nftState, "fee_percentage", parseString) || "0"),
  ...
  try {
    ...
    for (const entry of matchingEntries) {
      ...
      fees.push({
        ...
        amount: new Decimal4(entry.vaultAmount || "0"),
...
function checkAccountBondAffordability(balances, requiredAmount) {
  ...
  const requiredDecimal = new Decimal5(requiredAmount);
  for (const item of balances) {
    const balanceDecimal = new Decimal5(item.balance);
...
async function dispatchRegistrarRegistration({
  ...
  try {
    const feePercentageDecimal = feePercentage instanceof Decimal6 ? feePercentage : new Decimal6(feePercentage);
...
async function dispatchRegistrarUpdate({
  ...
  try {
    const feePercentageDecimal = feePercentage !== void 0 ? feePercentage instanceof Decimal7 ? feePercentage : new Decimal7(feePercentage) : void 0;
...
var NamespaceSDK = class {
  ...
  initGateway({ gateway, gatewayEndpoint }) {
    const gatewayInstance = gateway != null ? gateway : GatewayApiClient.initialize({
